...
#define MIN_DEGREE                                         \
    2 // Minimum degree (defines the range for number of
      // keys)
...
// Prototipo da função para exibir a árvore recursivamente
void exibirArvore(Node* no, int nivel);

void exibirArvoreBPlus(BTree* arvore) {
    if (!arvore || !arvore->root) {
        printf("A árvore está vazia.\n");
        return;
    }

    printf("Exibindo a árvore B+:\n");
...
void exibirArvore(Node* no, int nivel) {
    ...
    // Exibe os ponteiros para os filhos
    printf("  Ponteiros:\n");
    for (int i = 0; i <= no->n; i++) {
        printf("    Filho %d: %p\n", i + 1, (void*)no->children[i]);
    }

    // Se o nó não é folha, chama recursivamente para os filhos
    if (!no->leaf) {
        for (int i = 0; i <= no->n; i++) {
            exibirArvore(no->children[i], nivel + 1);
        }
    }
}
